...
def rule_single(tree):
    ...
    if len(r1) == 1:
        ...
        r = re.findall("\(CC (.*?)\)", tree, re.DOTALL)
        r1 = re.findall("\(ADVP \(RB (.*?)\)", tree, re.DOTALL)
        r1 += re.findall("\(WHNP \(WDT (.*?)\)", tree, re.DOTALL)
        r1 = r1 + r
        if len(r1) == 0:  # 2s + no advp ()
            ...
            if len(r1) == 0:  # 2s + no sbar (())
                ...
                if len(r1) == 0:  # 2s + no cc   ((()))
                    r2 = re.findall("\(IN", tree, re.DOTALL)
                    if len(r2) == 0:  # 2s + no in   ((((-)))) no dm
                        return 0
                    else:  # 2s + in      (((\())))
                ...
                r1 = [re.findall("\(SBAR(.*)\(IN (.*?)\)", tree, re.DOTALL)[0][1]]
            return r1  # 2s +( sbar    (\())
        else:
